''' find the following pattern UC UC UC lc UC UC UC '''
...
for i in range(len(weirdString)):
    if i+6 > len(weirdString):
        print("reached end")
        break
    if weirdString[i].isupper() and weirdString[i+1].isupper() and weirdString[i+2].isupper():
        if weirdString[i+3].islower():
            if weirdString[i+4].isupper() and weirdString[i+5].isupper() and weirdString[i+6].isupper():
                possibleSolution = weirdString[i:i+7]
                print(possibleSolution)
                #break

regexResult = re.findall("[^A-Z]+[A-Z]{3}([a-z]){1}[A-Z]{3}[^A-Z]+", weirdString) # filter more than 3 capitals and only match lowercase
print("".join(regexResult)) # gives linkedlist